...
> `createEntitySlice(adapterOptions?).setReducer(singular | [singular, plural], adjectives?, defaultCaseReducer?);`
...
> `extendAction(actionCreator, actionTransformer): ActionCreator;`

Creates a new action creator by extending the given enhanced action creator.
